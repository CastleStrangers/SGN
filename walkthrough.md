...
## 9. أتمتة جلب الأخبار تلقائياً في بيئة التطوير (Automatic Dev Sync)
* **الملف المعدل**: [route.ts](file:///d:/I-Ai/App/Syrian%20community%20in%20the%20Netherlands/SGN/src/app/api/news/route.ts)
  * تم إضافة رمز أتمتة داخل نهاية واجهة الأخبار (`GET /api/news`) يعمل فقط في بيئة التطوير (`process.env.NODE_ENV === "development"`).
  * بمجرد تشغيل خادم التطوير المحلي وفتح الصفحة الرئيسية للموقع، يتم إطلاق مهمة مزامنة تلقائية بالخلفية (Background Sync) لجلب أحدث المنشورات والأخبار من الموقع الرسمي ومن صفحة فيسبوك.
  * تم تزويد هذه المهمة بآلية خنق (Throttle) بمدة 5 دقائق كحد أقصى بين كل دورة مزامنة لمنع حدوث أي بطء في تحميل واجهة المستخدم.
  * تضمن هذه الآلية تحديث قاعدة البيانات وجلب التهنئة بالعام الهجري الجديد وغيرها من المنشورات فوراً عند تشغيل خادم التطوير.
