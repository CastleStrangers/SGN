    ...
    <div dir="auto" className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-white p-1 rounded-2xl border border-slate-100 shadow-sm flex gap-1">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all
                ${viewMode === "map" ? "bg-emerald-700 text-white shadow-lg" : "text-slate-400 hover:text-slate-600"}`}
            >
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          {/* Filters */}
          <div className={`${viewMode === "map" ? "lg:col-span-4" : "lg:col-span-12"} space-y-6`}>
            <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 p-4 flex flex-col gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none text-slate-700 text-sm"
                />
              </div>
              <div className="relative">
            ...
            {viewMode === "map" && !loading && (
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {members.map(m => (
                  <Link key={m.id} href={`/profile/${m.id}`} className="block bg-white p-4 rounded-2xl border border-slate-100 hover:border-emerald-200 transition-all">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-700 font-black shrink-0 overflow-hidden border">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-emerald-600 font-bold">{m.profession}</p>
                      </div>
          ...
          {/* Content Area */}
          <div className={`${viewMode === "map" ? "lg:col-span-8" : "lg:col-span-12"}`}>
            {viewMode === "map" ? (
              <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 p-8 h-full min-h-[600px] flex flex-col">
                <div className="flex items-center justify-between mb-8">
                  ...
                  {province && <span className="px-4 py-1.5 bg-emerald-50 text-emerald-700 rounded-full text-xs font-black border border-emerald-100">{province}</span>}
                </div>
              ...
              loading ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {members.map((member) => (
                    <div key={member.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group overflow-hidden">
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-16 h-16 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-700 text-2xl font-black shrink-0 overflow-hidden border border-emerald-100">
                          ...
                          <div className="flex flex-col items-end">
                            ...
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-[10px] font-bold uppercase tracking-wider border border-emerald-100">
                              {tm("accepted")}
                            </span>
                          </div>
                        </div>

                        <h3 className="text-lg font-black text-slate-900 group-hover:text-emerald-700 transition-colors mb-1">
                          {member.nameAr}
                        </h3>
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                          ...
                          <Link
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold transition-colors"
                          >
