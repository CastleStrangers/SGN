      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {showSuggestions && aiMessages.length === 0 && (
          <div className="text-center py-8">
            <Bot className="w-12 h-12 mx-auto mb-3 text-[#1a5632] opacity-60" />
            <p className="text-gray-900 font-medium text-sm mb-1">{t("aiTitle")}</p>
            <p className="text-xs text-gray-500 mb-4">{t("aiWelcome")}</p>
            <div className="flex flex-wrap justify-center gap-2 max-w-md mx-auto">
              {SUGGESTED_QUESTIONS.map((q) => (
                <button
                  key={q}
                  onClick={() => handleAISend(q)}
                  className="text-xs bg-gray-50 hover:bg-[#f0f7f2] text-gray-700 border px-3 py-1.5 rounded-full transition-colors"
                >
                  {q}
                </button>
              ))}
        ...
        {aiError && !aiLoading && aiMessages.length > 0 && (
