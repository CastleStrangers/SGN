    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={t("search")} className="w-full pr-10 p-3 border rounded-xl text-sm bg-white" />
        </div>
        <select value={filter} onChange={e => setFilter(e.target.value)} className="p-3 border rounded-xl text-sm bg-white">
        ...
        <label className="p-3 border rounded-xl text-sm bg-white hover:bg-gray-50 transition flex items-center gap-2 cursor-pointer whitespace-nowrap" title="استيراد Excel">
          ...
          <input type="file" accept=".csv,.txt" className="hidden" onChange={importExcel} disabled={importing} />
        </label>
      ...
      {selected.size > 0 && (
        <div className="mb-4 p-3 bg-blue-50 rounded-xl flex flex-wrap items-center gap-3 text-sm">
          ...
          <select value={batchStatus} onChange={e => setBatchStatus(e.target.value)} className="p-2 border rounded-lg text-sm bg-white">
            <option value="pending">قيد المراجعة</option>
            <option value="accepted">مقبول</option>
            <option value="rejected">مرفوض</option>
          </select>
          <input type="text" value={batchNotes} onChange={e => setBatchNotes(e.target.value)} placeholder="ملاحظات..." className="p-2 border rounded-lg text-sm flex-1 min-w-[120px]" />
