...
function NewsPageInner() {
  ...
  // تحديث الفئة النشطة عند تغيير URL params
  useEffect(() => {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#fafdfb] text-gray-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Horizontal Navigation Pills (Top of Feed) */}
          ...
          <div className="lg:col-span-3 order-1 lg:order-2">
            ...
            <div className="mb-8">
              <form onSubmit={e => { e.preventDefault(); fetchPosts(); }} className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="px-6 py-3 bg-[#1a5632] hover:bg-[#113d22] text-white rounded-2xl text-sm font-semibold shadow-sm transition-colors"
                >
                  بحث
                </button>
              </form>
                ...
                <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-6">
                  {posts.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className="bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-transparent hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 ease-out group flex flex-col relative"
                      >
                        {/* Image or YouTube thumbnail overlay */}
                        <div
                          className="block overflow-hidden relative aspect-video cursor-pointer"
                          onClick={(e) => handlePostClick(e, p)}
                        >
                          <img
                            src={getThumb(p)}
                            alt={p.title}
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                          <h3 className="font-bold text-gray-900 text-base line-clamp-2 leading-snug hover:text-[#1a5632] transition-colors duration-200">
                            {isVideo ? (
                              <button
                                onClick={(e) => handlePostClick(e as any, p)}
                                className="text-right font-bold w-full focus:outline-none"
                              >
                                {p.title}
                              </button>
                            ) : (
                              <Link href={`/news/${p.slug || p.id}`}>
                                {p.title}
                              </Link>
                            )}
                          </h3>
                    ...
                    {Array.from({ length: totalPages }, (_, i) => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-xl text-sm font-semibold transition-all ${
                          page === i
                            ? "bg-[#1a5632] text-white shadow-md shadow-[#1a5632]/20"
          ...
          <aside className="lg:col-span-1 order-2 lg:order-1 space-y-6">
              ...
              <div className="p-3 space-y-1">
                {CATEGORIES.map((cat) => (
                  <button
                    key={cat}
                    onClick={() => handleCategoryClick(cat)}
                    className={`block w-full text-right px-4 py-2.5 rounded-xl text-sm transition-all ${
                      activeCategory === cat
                        ? "bg-[#1a5632]/10 text-[#1a5632] font-bold"
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  { icon: Facebook, label: t('facebook'), color: "bg-blue-600 hover:bg-blue-700", href: "https://www.facebook.com/profile.php?id=61584301535331" },
                  { icon: Instagram, label: t('instagram'), color: "bg-gradient-to-tr from-yellow-500 via-pink-500 to-purple-600 hover:opacity-90", href: "https://www.instagram.com/sgn_syria/" },
                  { icon: Youtube, label: t('youtube'), color: "bg-red-600 hover:bg-red-700", href: "https://www.youtube.com/@SY-NL" },
                  { icon: Twitter, label: t('twitter'), color: "bg-gray-950 hover:bg-black", href: "https://x.com/SGN2098551" },
                  { icon: TikTok, label: t('tiktok'), color: "bg-gray-900 hover:bg-black", href: "https://www.tiktok.com/@sgn_syria" },
                ].map((s) => (
                  <a
                    key={s.label}
                    href={s.href}
                    ...
                    className={`${s.color} text-white rounded-xl px-3.5 py-2 flex items-center gap-2 text-xs font-medium shadow-sm transition-all hover:scale-102`}
                  >
            ...
            <div className="bg-gradient-to-br from-[#1a5632] to-[#0f3d23] text-white rounded-2xl p-6 text-center shadow-md relative overflow-hidden">
              ...
              <a
                href="https://docs.google.com/forms/d/e/1FAIpQLSckV1utDGcCXjB4Lg8p9hiYtJjgHwUJyTZb2waISC9dBZdKJw/viewform"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-block bg-[#c8a84e] hover:bg-[#b8973f] text-[#1a1a2e] font-bold px-5 py-2.5 rounded-xl text-xs shadow-md shadow-[#c8a84e]/10 transition-colors w-full"
              >
                {t('joinButton')}
              </a>
            </div>
      ...
      {/* Modern Pop-up YouTube Iframe Player Modal */}
      {activeVideoId && (
        <div
        ...
        >
          <div
            className="relative w-full max-w-4xl bg-[#0b0c10] rounded-3xl overflow-hidden shadow-2xl border border-white/10 animate-scale-in"
            onClick={e => e.stopPropagation()}
          >
            {/* Topbar of the player */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/5 backdrop-blur-sm">
              <span className="text-sm font-semibold text-[#c8a84e] flex items-center gap-2">
                ...
                <span>مشغل الفيديوهات الذكي</span>
              </span>
              <button
                onClick={() => setActiveVideoId(null)}
                className="p-2 rounded-xl bg-white/5 text-white/80 hover:bg-white/10 hover:text-white hover:scale-105 transition-all"
                aria-label="إغلاق"
              >
                <X className="w-5 h-5" />
              </button>
